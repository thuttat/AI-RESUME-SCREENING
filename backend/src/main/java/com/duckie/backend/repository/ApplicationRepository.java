...
public interface ApplicationRepository extends JpaRepository<Application, Long> {
        ...
        @Query("SELECT a FROM Application a WHERE (:jobId IS NULL OR a.jobPosting.id = :jobId) AND (:status IS NULL OR a.status = :status)")
        Page<Application> findByJobPostingIdAndStatus(@Param("jobId") Long jobId, @Param("status") Status status, Pageable pageable);

        @Query("SELECT a FROM Application a JOIN a.cv c JOIN c.aiAnalysisResult ai WHERE a.jobPosting.id = :jobId ORDER BY ai.matchScore DESC")
        ...
        @Query("SELECT CASE WHEN COUNT(a) > 0 THEN true ELSE false END FROM Application a WHERE a.cv.candidateEmail = :email AND a.jobPosting.id = :jobId AND a.cv.id != :currentCvId")
        boolean existsDuplicateByEmailForJob(@Param("email") String email, @Param("jobId") Long jobId, @Param("currentCvId") Long currentCvId);

        Page<Application> findByJobPostingId(Long jobId, Pageable pageable);

        @Query("SELECT FUNCTION('MONTHNAME', a.createdAt) as month, COUNT(a) as applications FROM Application a WHERE a.jobPosting.createdBy.username = :username GROUP BY FUNCTION('MONTHNAME', a.createdAt), MONTH(a.createdAt) ORDER BY MONTH(a.createdAt) ASC")
        ...
        @Query("SELECT a.jobPosting.title, COUNT(a) FROM Application a GROUP BY a.jobPosting.title")
